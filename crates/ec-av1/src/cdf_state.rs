...
pub(crate) enum TxbSet {
    ...
    Luma4,
    /// The 4x4 luma transform of an `is_inter` block whose `TxMode::Select`
    /// var-tx tree splits an 8x8 unit all the way down (lane-txselect): the
    /// same coefficient tables [`Luma4`](TxbSet::Luma4) reads (libaom's
    /// coefficient CDFs are keyed by plane type, not intra/inter), with the
    /// inter `tx_type` set in place of the intra one.
    Luma4Inter,
    /// [`Luma4Inter`](TxbSet::Luma4Inter)'s `reduced_tx_set: false`
    /// counterpart -- the sixteen-symbol `EXT_TX_SET_ALL16` table
    /// ([`crate::cdf::INTER_TX_TYPE_SET1_4`]).
    Luma4InterSet1,
    ...
    /// class down.
    ChromaRect8x4,
    /// The luma 4x8/8x4 transform of a `PARTITION_VERT`/`HORZ` leaf under an
    /// 8x8 block (lane-tx4x8), under this encoder's `reduced_tx_set`.
    /// `get_txsize_entropy_ctx` ((`txsize_sqr_map` + `txsize_sqr_up_map` + 1)
    /// >> 1) resolves `TX_4X8` to `TX_8X8`, so every coefficient table here is
    /// [`Luma8`](TxbSet::Luma8)'s -- except `eob_pt`, a true 32-position
    /// alphabet ([`crate::cdf::EOB_PT_32_LUMA`]), and `tx_type`, whose CDF row
    /// is `txsize_sqr_map[TX_4X8]` = `TX_4X4` (libaom `read_tx_type`'s
    /// `square_tx_size`), i.e. [`Luma4`](TxbSet::Luma4)'s own table.
    LumaRect4x8,
    /// [`LumaRect4x8`](TxbSet::LumaRect4x8)'s `reduced_tx_set: false`
    /// counterpart: `av1_get_ext_tx_set_type` at `tx_size_sqr_up == TX_8X8`,
    /// `tx_size_sqr == TX_4X4`, intra returns `EXT_TX_SET_DTT4_IDTX_1DDCT`,
    /// the same seven-symbol set (and same `TX_4X4` row) `Luma4Set1` reads.
    LumaRect4x8Set1,
    /// The luma 16x4/4x16 transform of a 16x16-level `PARTITION_HORZ_4`/
    /// `PARTITION_VERT_4` strip (lane-r14). `get_txsize_entropy_ctx`
    /// ((`txsize_sqr_map` + `txsize_sqr_up_map` + 1) >> 1) resolves `TX_16X4`
    /// to `TX_8X8`, and the transform has 64 coded positions, so EVERY table
    /// here is [`Luma8`](TxbSet::Luma8)'s (`eob_pt_64` included) except
    /// `tx_type`: `read_tx_type` indexes `intra_ext_tx_cdf[eset]
    /// [txsize_sqr_map[TX_16X4]]` = the `TX_4X4` row, and
    /// `av1_get_ext_tx_set_type` at `tx_size_sqr_up == TX_16X16` with
    /// `tx_size_sqr == TX_4X4` returns `EXT_TX_SET_DTT4_IDTX` under
    /// `reduced_tx_set` -- the same pair [`LumaRect4x8`](TxbSet::LumaRect4x8)
    /// reads, which differs from this variant only in its 32-position `eob_pt`.
    LumaRect16x4,
    /// [`LumaRect16x4`](TxbSet::LumaRect16x4)'s `reduced_tx_set: false`
    /// counterpart: `EXT_TX_SET_DTT4_IDTX_1DDCT`, the seven-symbol `TX_4X4`
    /// row [`LumaRect4x8Set1`](TxbSet::LumaRect4x8Set1) reads.
    LumaRect16x4Set1,
    /// The luma 8x4/4x8 transform of an INTER `PARTITION_HORZ`/`PARTITION_VERT`
    /// leaf under an 8x8 block (lane-intersub8 r3): every coefficient table is
    /// [`LumaRect4x8`](TxbSet::LumaRect4x8)'s (`get_txsize_entropy_ctx` squares
    /// `TX_8X4` up to the 8x8 class, 32-position `eob_pt`), with the INTER
    /// `tx_type` alphabet in its place. Under `reduced_tx_set`
    /// `av1_get_ext_tx_set_type` (`tx_size_sqr_up == TX_8X8`, `is_inter`)
    /// returns `EXT_TX_SET_DCT_IDTX`, whose row is
    /// `default_inter_ext_tx_cdf[?][txsize_sqr_map[TX_8X4] == TX_4X4]` -- the
    /// same `TX_4X4` inter row [`Luma4Inter`](TxbSet::Luma4Inter) reads.
    LumaRect8x4Inter,
    /// [`LumaRect8x4Inter`](TxbSet::LumaRect8x4Inter)'s `reduced_tx_set: false`
    /// counterpart: `tx_size_sqr == TX_4X4` with `tx_size_sqr_up == TX_8X8`, so
    /// `av1_ext_tx_set_lookup[is_inter=1][...]` is `EXT_TX_SET_ALL16` -- sixteen
    /// symbols at the `TX_4X4` inter row, the table
    /// [`Luma4InterSet1`](TxbSet::Luma4InterSet1) also reads.
    LumaRect8x4InterSet1,
    /// [`LumaRect16x4`](TxbSet::LumaRect16x4)'s INTER counterpart
    /// (lane-inter16ab r2): the same `Luma8` coefficient tables (64 coded
    /// positions, `get_txsize_entropy_ctx(TX_16X4)` = TX_8X8), with the
    /// INTER `tx_type` table at `txsize_sqr_map[TX_16X4]` = TX_4X4.
    /// `av1_get_ext_tx_set_type` (av1/common/idct.c): `tx_size_sqr_up` is
    /// TX_16X16 (not > TX_32X32) and `use_reduced_set` picks
    /// `EXT_TX_SET_DCT_IDTX` -- three symbols, `inter_tx_type_4`.
    LumaRect16x4Inter,
    /// [`LumaRect16x4Inter`](TxbSet::LumaRect16x4Inter)'s
    /// `reduced_tx_set: false` counterpart: `tx_size_sqr` is TX_4X4, not
    /// TX_16X16, so `av1_get_ext_tx_set_type` returns `EXT_TX_SET_ALL16` --
    /// sixteen symbols, `inter_tx_type_4_set1`.
    LumaRect16x4Inter1,
    /// The luma 8x4/4x8 INTER transform of a `sub_tx_size_map` SPLIT LEAF of
    /// a 16x4/4x16 strip (lane-inter16ab r4): `sub_tx_size_map[TX_16X4] ==
    /// TX_8X4`. Shape = [`LumaRect4x8`](TxbSet::LumaRect4x8)'s --
    /// `get_txsize_entropy_ctx(TX_8X4)` ((`txsize_sqr_map` TX_4X4 +
    /// `txsize_sqr_up_map` TX_8X8 + 1) >> 1) resolves to TX_8X8 so every
    /// coefficient table is [`Luma8`](TxbSet::Luma8)'s, with the true
    /// 32-position `eob_pt` -- but the `tx_type` table is the INTER one at
    /// `txsize_sqr_map[TX_8X4]` = TX_4X4. `av1_get_ext_tx_set_type`
    /// (`blockd.h:1097`) with `tx_size_sqr_up == TX_8X8` (not > TX_32X32),
    /// inter and `use_reduced_set` returns `EXT_TX_SET_DCT_IDTX` --
    /// `inter_tx_type_4`.
    /// [`LumaRect8x4Inter`](TxbSet::LumaRect8x4Inter)'s `reduced_tx_set:
    /// false` counterpart: `tx_size_sqr` is TX_4X4, so
    /// `av1_get_ext_tx_set_type` returns `EXT_TX_SET_ALL16` --
    /// `inter_tx_type_4_set1`.
    LumaRect8x4Inter1,
}
...
pub(crate) struct Cdfs {
    ...
    /// `txfm_partition_cdf`: an inter block's `txfm_split` flag under
    /// `TxMode::Select` (lane-txselect, spec 5.11.17), indexed by
    /// `txfm_partition_context` -- see [`cdf::TXFM_PARTITION`].
    pub txfm_partition: [[u16; 3]; 21],
    pub skip: [[u16; 3]; 3],
    ...
    /// `eob_pt_128_luma`'s class-1 (`TX_CLASS_HORIZ`/`VERT`) sibling, see `eob_pt_16_luma_class1`.
    pub eob_pt_128_luma_class1: [u16; 9],
    /// `eob_pt_32_chroma`'s class-1 (`TX_CLASS_HORIZ`/`VERT`) sibling, see `eob_pt_16_luma_class1`.
    pub eob_pt_32_chroma_class1: [u16; 7],
    ...
    /// The end-of-block group of a 32-position (4x8/8x4) luma transform
    /// (lane-tx4x8).
    pub eob_pt_32_luma: [u16; 7],
    /// `eob_pt_32_luma`'s class-1 sibling, see `eob_pt_16_luma_class1`.
    pub eob_pt_32_luma_class1: [u16; 7],
    ...
    /// The 4x4 counterparts of [`Self::inter_tx_type_8`]/
    /// [`Self::inter_tx_type_8_set1`] (lane-txselect).
    pub inter_tx_type_4: [u16; 3],
    /// See [`Self::inter_tx_type_4`].
    pub inter_tx_type_4_set1: [u16; 17],
    ...
    /// `[6]`=16x8, `[7]`=8x16, `[8]`=4x8, `[9]`=8x4, `[10]`=32x8, `[11]`=8x32) --
    /// see [`cdf::FILTER_INTRA`].
    pub filter_intra: [[u16; 3]; 14],
...
impl Cdfs {
    ...
    pub(crate) fn reset_counts(&mut self) {
        ...
        reset1(&mut self.eob_pt_128_luma_class1);
        reset1(&mut self.eob_pt_32_chroma_class1);
        ...
        reset1(&mut self.eob_pt_32_luma);
        reset1(&mut self.eob_pt_32_luma_class1);
        ...
        reset1(&mut self.inter_tx_type_4);
        reset1(&mut self.inter_tx_type_4_set1);
    ...
    pub fn new(q_ctx: usize) -> Cdfs {
        Cdfs {
            ...
            eob_pt_128_luma_class1: pick(
                q_ctx,
                cdf::EOB_PT_128_LUMA_CLASS1_Q0,
                cdf::EOB_PT_128_LUMA_CLASS1_Q1,
                cdf::EOB_PT_128_LUMA_CLASS1,
                cdf::EOB_PT_128_LUMA_CLASS1_Q3,
            ),
            eob_pt_32_chroma_class1: pick(
                q_ctx,
                cdf::EOB_PT_32_CHROMA_CLASS1_Q0,
                cdf::EOB_PT_32_CHROMA_CLASS1_Q1,
                cdf::EOB_PT_32_CHROMA_CLASS1,
                cdf::EOB_PT_32_CHROMA_CLASS1_Q3,
            ),
            ...
            inter_tx_type_4: cdf::INTER_TX_TYPE_SET3_4,
            inter_tx_type_4_set1: cdf::INTER_TX_TYPE_SET1_4,
            ...
            txfm_partition: cdf::TXFM_PARTITION,
    ...
    pub fn txb(&mut self, set: TxbSet, mode: usize) -> TxbTables<'_> {
        match set {
            ...
            TxbSet::Luma4InterSet1 => TxbTables {
                side: 4,
                txb_skip: &mut self.txb_skip_luma_4,
                eob_pt: &mut self.eob_pt_16_luma,
                eob_extra: &mut self.eob_extra_luma_4,
            ...
            TxbSet::LumaRect8x4InterSet1 => TxbTables {
                side: 8,
                txb_skip: &mut self.txb_skip_luma_8,
                eob_pt: &mut self.eob_pt_32_luma,
                eob_extra: &mut self.eob_extra_luma_8,
            ...
            TxbSet::LumaRect4x8Set1 => TxbTables {
                side: 8,
                txb_skip: &mut self.txb_skip_luma_8,
                eob_pt: &mut self.eob_pt_32_luma,
                eob_extra: &mut self.eob_extra_luma_8,
            ...
            TxbSet::LumaRect16x4Set1 => TxbTables {
                side: 8,
                txb_skip: &mut self.txb_skip_luma_8,
                eob_pt: &mut self.eob_pt_64_luma,
                eob_extra: &mut self.eob_extra_luma_8,
            ...
            TxbSet::LumaRect16x4Inter => TxbTables {
                side: 8,
                txb_skip: &mut self.txb_skip_luma_8,
                eob_pt: &mut self.eob_pt_64_luma,
                eob_extra: &mut self.eob_extra_luma_8,
            ...
            TxbSet::LumaRect16x4Inter1 => TxbTables {
                side: 8,
                txb_skip: &mut self.txb_skip_luma_8,
                eob_pt: &mut self.eob_pt_64_luma,
                eob_extra: &mut self.eob_extra_luma_8,
            ...
            TxbSet::ChromaRect8x4 => TxbTables {
                ...
                dc_sign: &mut self.dc_sign_chroma,
                tx_type: None,
                eob_pt_class1: Some(&mut self.eob_pt_32_chroma_class1),
            },
        }
    }
}
