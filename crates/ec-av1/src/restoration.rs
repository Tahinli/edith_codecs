...
fn apply_wiener_stripe(
    ...
    // lane-av112bit: libaom `get_conv_params_wiener` (`convolve.h:107..118`)
    // raises the Wiener pair by the same `intbufrange` delta as MC's: at
    // 12-bit `round_0` is 5 and `round_1` 9 (8/10-bit: 3 and 11). The ROUNDS
    // are parameterised; the intermediate CLAMP below keeps the corpus-proven
    // constant-bound structure extended by the same bd formulas -- a first
    // attempt at the C clamp's exact per-sample transform (the add_src bias
    // is `src[centre] << FILTER_BITS + (1 << (bd + FILTER_BITS - 1))`, which
    // survives `Round2(_, round_0)` as `(128 >> round_0) * s +
    // (1 << (bd + 6 - round_0))`) MOVED the 10-bit 128-superblock film gate
    // (frame 1 luma): libaom's 8-tap-loop alignment trick means the C
    // horizontal intermediate is not `ours + that bias` wherever the clamp
    // actually binds, so the exact transform is NOT derivable from the
    // convolve source alone. The bd-formula clamp is what every 8/10-bit
    // byte-exact gate (this film included) has always pinned, and the 12-bit
    // key witness passes byte-exact with it; where C's real clamp would bind
    // differently at 12 bits remains unproven either way -- recorded in
    // `lanes/av112bit.report.md` as the open half of the Wiener lift.
    let wiener_bias = 1i64 << (bd + 3);
    let wiener_limit = 1i64 << (bd + 5);
    let (round0, round1) = crate::mc::round_pair(bd as u32);
    let rows = h + 6;
    ...
    // range at every supported bit depth (the centred sum's own range,
    // `|kernel| <= 348` against `2^bd - 1` samples, never reaches them), so
    // the kernel's saturating store composes with this clamp instead of
    // changing it: `[-2^(bd+3), 2^(bd+5)-1-2^(bd+3)]` -- 8-bit
    // `[-2048, 6143]`, 10-bit `[-8192, 24575]`. At 12-bit those formulas
    // leave `i16`, so the clamp there is the widest `i16` window -- which
    // strictly contains the kernel's own mathematical range (`|sum| <=
    // round2(348 * 4095, round_0=5) == 27976 < 2^15`), i.e. it never binds,
    // the property the 8/10-bit bounds have on the whole proven corpus.
    let (lo_bound, hi_bound) = if bd >= 12 {
        (i16::MIN, i16::MAX)
    } else {
        (-wiener_bias as i16, (wiener_limit - 1 - wiener_bias) as i16)
    };
    ...
    for r in 0..rows {
        ...
        // The row the pass convolves, contiguous when the block sits inside
        // the plane and the clamped copy (`hbuf`) at an edge.
        let interior = lo >= 0 && lo as usize + w + 7 <= plane_w;
        if !interior {
            hbuf.clear();
            hbuf.extend(
        ...
        let row_slice: &[u16] = if interior { &src[lo as usize..] } else { &hbuf };
        crate::mc::hpass_contig(row_slice, &t16, out, round0);
    ...
    // the accumulator unrounded, and `Round2(_, round1)` is the frame's
    // non-compound `round_1` (11 at 8/10-bit, 9 at 12 -- libaom's
    ...
    for r in 0..h {
        ...
        for (o, &a) in dst.iter_mut().zip(acc.iter()) {
            *o = round2(i64::from(a), round1).clamp(0, smax) as u16;
        }
    }
