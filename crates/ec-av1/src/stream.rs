...
mod tests {
    ...
    ///
    #[test]
    #[ignore = "r7: the symbol-level cross-check this note asked for is done and CLEARS the coefficient decode. An EC_AV1_TRACE-gated trace (decode.rs read_coeffs/read_eob) was diffed against an independent Python MSAC decoder (real libaom default CDFs pulled from token_cdfs.h, context arithmetic ported from txb_common.h's get_nz_mag/get_nz_map_ctx_from_stats/get_br_ctx -- not from this crate). Every symbol matches ours bit-for-bit through the full Luma32 base loop: eob_pt=5, eob_extra(ctx=2)=1, eob=15, base_eob(ctx=1)=1, then base ctx/value pairs 21/0,21/0,21/0,21/0,6/0,6/0,6/0,7/0,7/2,6/0,6/0,1/0,2/2,0/2, dc_sign(ctx=0)=negative -- three nonzero levels (magnitude 2 each) at raster positions 0/1/2. The scan table, CDF tables, and every context function are proven correct for this stream; r6's transposition hypothesis stays reverted and should not be revisited. Feeding those exact levels through dequant_and_inverse_typed (base_q_idx=60) gives a near-flat residual (row 0 barely varies, rows 1-31 replicate it -- correct behaviour for energy confined to row 0) landing luma at a uniform ~126 against ffmpeg's real ~50-and-varying output: three magnitude-2 coefficients cannot close a gap that size even transformed exactly right, so the desync is BEFORE this trace starts -- mode_info (partition/skip/y_mode/uv_mode) or the tile-group byte offset itself. r8: extend EC_AV1_TRACE (already added, off by default) to mode_info's own symbols and cross-check them the same way, and separately confirm the tile data's starting byte offset against the OBU/frame-header parse (ec-av1-syntax) rather than trusting a byte slice pulled from a prior round's own decoder run."]
