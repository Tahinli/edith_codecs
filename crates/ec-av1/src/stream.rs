...
mod tests {
    ...
    ///
    #[test]
    #[ignore = "r6: scan-transposition hypothesis FALSIFIED empirically, not by re-derivation. Read `default_scan_4x4/8x8/32x32` in /tmp/libaom-src/av1/common/scan.c as raster indices (value = row*side+col) and found the odd/even diagonal-walk parity backwards vs this crate's default_scan in both tile.rs and decode.rs; swapped both. Result: a_real_libaom_gradients_stream_decodes_pixel_exact (the whole-SB Luma64/TX32 case, previously ground-truth pixel-exact against ffmpeg per r5) REGRESSED to failing, and this CfL test's failure was UNCHANGED (still wrong, same row-0-only energy shape) -- reverted both files back to HEAD (5232eff). Two readings are possible: either libaom's default_scan_NxN array is not read as row-major (row*side+col) the way I assumed -- e.g. it may be column-major (col*side+row) or defined over a transposed coefficient buffer relative to this crate's grid[row*side+col] -- or the parity truly is backwards but only for Luma32 BLOCK-granularity quadrants specifically, not the Luma64 whole-SB case, meaning a single default_scan() cannot serve both correctly and the bug is a missing quadrant-position transform (block-relative vs SB-relative row/col), not the scan generator itself. r7 should NOT re-attempt a source-transcription parity re-derivation without first re-confirming against gradients (known-good, keep it green) on every candidate; instead resume r6's extend-the-python-cross-check plan (msac.py has the CDF_TOP/EC_PROB_SHIFT MSAC core through the eob-group symbol already) through base/base_eob/br/dc_sign/golomb for the pinned CfL block, comparing context index and decoded value at every step against a temporary Rust trace (EC_AV1_TRACE pattern, strip before commit), to find the first (ctx, value) divergence directly rather than guessing at table conventions."]
