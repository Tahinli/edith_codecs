...
const PROVEN: &[(&str, &str)] = &[
    ...
    // refused by name with no picture; and real aomenc recipes never enable
    // one. lane-av1seglvl re-scoped the census: beyond r27's six 1-pass
    // no-lag recipes it now parses EVERY frame header's feature table over
    // alt-ref-lag and genuine two-pass arms (`--aq-mode=1/2`, cq45 and the
    // branch's own high-q cq63, screen + film sources) -- the shapes that
    // reach `configure_static_seg_features`, the only setter of ALT_LF/
    // REF_FRAME/SKIP in the tree, which is gated on `hl_sf.static_segmentation`
    // ("Always set to 0", speed_features.h) and the second pass
    // (encoder_utils.c:738). What aomenc codes is `SEG_LVL_ALT_Q` tables and
