...
/// `[4]`/`[5]` (lane-intradisp r1) add `BLOCK_32X16`/`BLOCK_16X32`'s own rows
...
/// `default_palette_y_color_index_cdf` (entropymode.c): the per-pixel colour
/// index symbol for a palette-Y block, indexed `[n-2][color_ctx]` where `n`
/// is the block's palette size (2..=8, `PALETTE_SIZES`) and `color_ctx` is
/// `av1_get_palette_color_index_context`'s 0..=4 output
/// (`PALETTE_COLOR_INDEX_CONTEXTS`). Each row is transcribed straight from
/// its `AOM_CDFn` macro (`n-1` raw probs, then `32768, 0`) and zero-padded
/// out to width 9 (the `n=8` row's real length); the call site slices
/// `[..=n]`, which only ever touches the real prefix, so the trailing zero
/// padding on shorter rows is never read.
pub const PALETTE_Y_COLOR_INDEX: [[[u16; 9]; 5]; 7] = [
    [
...
/// `default_palette_uv_color_index_cdf` (entropymode.c): as
/// [`PALETTE_Y_COLOR_INDEX`], for the chroma colour-index map.
pub const PALETTE_UV_COLOR_INDEX: [[[u16; 9]; 5]; 7] = [
    [
...
/// `default_compound_type_cdf` (spec 9.4, libaom `entropymode.c`):
/// `compound_type` (`MASKED_COMPOUND_TYPES` == 2: `COMPOUND_WEDGE` vs
/// `COMPOUND_DIFFWTD`), indexed by bsize over `BLOCK_SIZES_ALL`, read only
/// on the `comp_group_idx == 1` (masked-compound) path -- lane-maskcomp.
pub const COMPOUND_TYPE: [[u16; 3]; 22] = [
    [16384, 32768, 0],
    [16384, 32768, 0],
    [16384, 32768, 0],
    [23431, 32768, 0],
...
/// `default_wedge_idx_cdf` (spec 9.4, libaom `entropymode.c`): `wedge_index`
/// (16-ary, `MAX_WEDGE_TYPES`), indexed by bsize over `BLOCK_SIZES_ALL` --
/// read only when `interinter_comp.type == COMPOUND_WEDGE` (lane-maskcomp).
pub const WEDGE_IDX: [[u16; 17]; 22] = [
    [
...
// q-context 0/1/3 coefficient CDFs (spec 9.4's `Default_..._Cdf[0]`,
// `[1]` and `[3]`), auto-derived from the same libaom `token_cdfs.h` the
// q-context-2 tables above were transcribed from, and checked by reproducing
// every q-context-2 table above byte-for-byte with the same indexing before
// generating these (see the lane report for the check).

// q-context 0: base_q_idx 0..=20.

/// TXB_SKIP_LUMA_64, q-context 0: the same table as [`TXB_SKIP_LUMA_64`], for `base_q_idx` 0..=20.
pub const TXB_SKIP_LUMA_64_Q0: [u16; 3] = [6308, 32768, 0];
/// TXB_SKIP_LUMA_32, q-context 0: the same table as [`TXB_SKIP_LUMA_32`], for `base_q_idx` 0..=20.
pub const TXB_SKIP_LUMA_32_Q0: [u16; 3] = [17920, 32768, 0];
/// TXB_SKIP_LUMA_16, q-context 0: the same table as [`TXB_SKIP_LUMA_16`], for `base_q_idx` 0..=20.
pub const TXB_SKIP_LUMA_16_Q0: [u16; 3] = [29957, 32768, 0];
/// TXB_SKIP_CHROMA_8, q-context 0: the same table as [`TXB_SKIP_CHROMA_8`], for `base_q_idx` 0..=20.
pub const TXB_SKIP_CHROMA_8_Q0: [[u16; 3]; 3] =
    [[5403, 32768, 0], [18096, 32768, 0], [30003, 32768, 0]];
/// TXB_SKIP_CHROMA_16, q-context 0: the same table as [`TXB_SKIP_CHROMA_16`], for `base_q_idx` 0..=20.
pub const TXB_SKIP_CHROMA_16_Q0: [[u16; 3]; 3] =
    [[3778, 32768, 0], [15336, 32768, 0], [28981, 32768, 0]];
/// TXB_SKIP_CHROMA_32, q-context 0: the same table as [`TXB_SKIP_CHROMA_32`], for `base_q_idx` 0..=20.
pub const TXB_SKIP_CHROMA_32_Q0: [[u16; 3]; 3] =
    [[1366, 32768, 0], [15628, 32768, 0], [30462, 32768, 0]];
/// EOB_EXTRA_LUMA_16, q-context 0: the same table as [`EOB_EXTRA_LUMA_16`], for `base_q_idx` 0..=20.
pub const EOB_EXTRA_LUMA_16_Q0: [[u16; 3]; 9] = [
    [23905, 32768, 0],
    [17194, 32768, 0],
    [16170, 32768, 0],
    [17695, 32768, 0],
...
/// EOB_EXTRA_LUMA_32, q-context 0: the same table as [`EOB_EXTRA_LUMA_32`], for `base_q_idx` 0..=20.
pub const EOB_EXTRA_LUMA_32_Q0: [[u16; 3]; 9] = [
    [27399, 32768, 0],
    [16327, 32768, 0],
    [18071, 32768, 0],
    [19584, 32768, 0],
...
/// EOB_EXTRA_LUMA_64, q-context 0: the same table as [`EOB_EXTRA_LUMA_64`], for `base_q_idx` 0..=20.
pub const EOB_EXTRA_LUMA_64_Q0: [[u16; 3]; 9] = [
    [23406, 32768, 0],
    [21845, 32768, 0],
    [18432, 32768, 0],
    [16384, 32768, 0],
...
/// EOB_EXTRA_CHROMA_8, q-context 0: the same table as [`EOB_EXTRA_CHROMA_8`], for `base_q_idx` 0..=20.
pub const EOB_EXTRA_CHROMA_8_Q0: [[u16; 3]; 9] = [
    [20681, 32768, 0],
    [20701, 32768, 0],
    [15250, 32768, 0],
    [15017, 32768, 0],
...
/// EOB_EXTRA_CHROMA_16, q-context 0: the same table as [`EOB_EXTRA_CHROMA_16`], for `base_q_idx` 0..=20.
pub const EOB_EXTRA_CHROMA_16_Q0: [[u16; 3]; 9] = [
    [23959, 32768, 0],
    [20799, 32768, 0],
    [19021, 32768, 0],
    [16203, 32768, 0],
...
/// EOB_EXTRA_CHROMA_32, q-context 0: the same table as [`EOB_EXTRA_CHROMA_32`], for `base_q_idx` 0..=20.
pub const EOB_EXTRA_CHROMA_32_Q0: [[u16; 3]; 9] = [
    [24932, 32768, 0],
    [20833, 32768, 0],
    [12027, 32768, 0],
    [16670, 32768, 0],
...
/// EOB_PT_1024_LUMA, q-context 0: the same table as [`EOB_PT_1024_LUMA`], for `base_q_idx` 0..=20.
pub const EOB_PT_1024_LUMA_Q0: [u16; 12] = [
    393, 421, 751, 1623, 3160, 6352, 13345, 18047, 22571, 25830, 32768, 0,
];
/// EOB_PT_1024_CHROMA, q-context 0: the same table as [`EOB_PT_1024_CHROMA`], for `base_q_idx` 0..=20.
pub const EOB_PT_1024_CHROMA_Q0: [u16; 12] = [
    1865, 1988, 2930, 4242, 10533, 16538, 21354, 27255, 28546, 31784, 32768, 0,
];
/// EOB_PT_256_LUMA, q-context 0: the same table as [`EOB_PT_256_LUMA`], for `base_q_idx` 0..=20.
pub const EOB_PT_256_LUMA_Q0: [u16; 10] =
    [310, 584, 1887, 3589, 6168, 8611, 11352, 15652, 32768, 0];
/// EOB_PT_256_CHROMA, q-context 0: the same table as [`EOB_PT_256_CHROMA`], for `base_q_idx` 0..=20.
pub const EOB_PT_256_CHROMA_Q0: [u16; 10] =
    [2520, 3240, 5952, 8870, 12577, 17558, 19954, 24168, 32768, 0];
/// EOB_PT_64_CHROMA, q-context 0: the same table as [`EOB_PT_64_CHROMA`], for `base_q_idx` 0..=20.
pub const EOB_PT_64_CHROMA_Q0: [u16; 8] = [3505, 5304, 10086, 13814, 17684, 23370, 32768, 0];
/// COEFF_BASE_LUMA_16, q-context 0: the same table as [`COEFF_BASE_LUMA_16`], for `base_q_idx` 0..=20.
pub const COEFF_BASE_LUMA_16_Q0: [[u16; 5]; 42] = [
...
/// COEFF_BASE_LUMA_32, q-context 0: the same table as [`COEFF_BASE_LUMA_32`], for `base_q_idx` 0..=20.
pub const COEFF_BASE_LUMA_32_Q0: [[u16; 5]; 42] = [
...
/// COEFF_BASE_LUMA_64, q-context 0: the same table as [`COEFF_BASE_LUMA_64`], for `base_q_idx` 0..=20.
pub const COEFF_BASE_LUMA_64_Q0: [[u16; 5]; 42] = [
...
/// COEFF_BASE_CHROMA_8, q-context 0: the same table as [`COEFF_BASE_CHROMA_8`], for `base_q_idx` 0..=20.
pub const COEFF_BASE_CHROMA_8_Q0: [[u16; 5]; 42] = [
...
/// COEFF_BASE_CHROMA_16, q-context 0: the same table as [`COEFF_BASE_CHROMA_16`], for `base_q_idx` 0..=20.
pub const COEFF_BASE_CHROMA_16_Q0: [[u16; 5]; 42] = [
...
/// COEFF_BASE_CHROMA_32, q-context 0: the same table as [`COEFF_BASE_CHROMA_32`], for `base_q_idx` 0..=20.
pub const COEFF_BASE_CHROMA_32_Q0: [[u16; 5]; 42] = [
...
/// COEFF_BR_LUMA_16, q-context 0: the same table as [`COEFF_BR_LUMA_16`], for `base_q_idx` 0..=20.
pub const COEFF_BR_LUMA_16_Q0: [[u16; 5]; 21] = [
...
/// COEFF_BR_LUMA_32, q-context 0: the same table as [`COEFF_BR_LUMA_32`], for `base_q_idx` 0..=20.
pub const COEFF_BR_LUMA_32_Q0: [[u16; 5]; 21] = [
...
/// COEFF_BR_CHROMA_8, q-context 0: the same table as [`COEFF_BR_CHROMA_8`], for `base_q_idx` 0..=20.
pub const COEFF_BR_CHROMA_8_Q0: [[u16; 5]; 21] = [
...
/// COEFF_BR_CHROMA_16, q-context 0: the same table as [`COEFF_BR_CHROMA_16`], for `base_q_idx` 0..=20.
pub const COEFF_BR_CHROMA_16_Q0: [[u16; 5]; 21] = [
...
/// COEFF_BR_CHROMA_32, q-context 0: the same table as [`COEFF_BR_CHROMA_32`], for `base_q_idx` 0..=20.
pub const COEFF_BR_CHROMA_32_Q0: [[u16; 5]; 21] = [
...
/// COEFF_BASE_EOB_LUMA_16, q-context 0: the same table as [`COEFF_BASE_EOB_LUMA_16`], for `base_q_idx` 0..=20.
pub const COEFF_BASE_EOB_LUMA_16_Q0: [[u16; 4]; 4] = [
    [1786, 12612, 32768, 0],
    [30663, 31625, 32768, 0],
    [32339, 32468, 32768, 0],
    [31148, 31833, 32768, 0],
];
/// COEFF_BASE_EOB_LUMA_32, q-context 0: the same table as [`COEFF_BASE_EOB_LUMA_32`], for `base_q_idx` 0..=20.
pub const COEFF_BASE_EOB_LUMA_32_Q0: [[u16; 4]; 4] = [
    [1787, 2532, 32768, 0],
    [30832, 31662, 32768, 0],
    [31824, 32682, 32768, 0],
    [32133, 32569, 32768, 0],
];
/// COEFF_BASE_EOB_LUMA_64, q-context 0: the same table as [`COEFF_BASE_EOB_LUMA_64`], for `base_q_idx` 0..=20.
pub const COEFF_BASE_EOB_LUMA_64_Q0: [[u16; 4]; 4] = [
    [1725, 3449, 32768, 0],
    [31102, 31935, 32768, 0],
    [32457, 32613, 32768, 0],
    [32412, 32649, 32768, 0],
];
/// COEFF_BASE_EOB_CHROMA_8, q-context 0: the same table as [`COEFF_BASE_EOB_CHROMA_8`], for `base_q_idx` 0..=20.
pub const COEFF_BASE_EOB_CHROMA_8_Q0: [[u16; 4]; 4] = [
    [12608, 27820, 32768, 0],
    [30680, 32225, 32768, 0],
    [30809, 32335, 32768, 0],
    [31299, 32423, 32768, 0],
];
/// COEFF_BASE_EOB_CHROMA_16, q-context 0: the same table as [`COEFF_BASE_EOB_CHROMA_16`], for `base_q_idx` 0..=20.
pub const COEFF_BASE_EOB_CHROMA_16_Q0: [[u16; 4]; 4] = [
    [18857, 23865, 32768, 0],
    [31428, 32428, 32768, 0],
    [31744, 32373, 32768, 0],
    [31775, 32526, 32768, 0],
];
/// COEFF_BASE_EOB_CHROMA_32, q-context 0: the same table as [`COEFF_BASE_EOB_CHROMA_32`], for `base_q_idx` 0..=20.
pub const COEFF_BASE_EOB_CHROMA_32_Q0: [[u16; 4]; 4] = [
    [13751, 22235, 32768, 0],
    [32089, 32409, 32768, 0],
    [27084, 27920, 32768, 0],
    [29291, 32594, 32768, 0],
...
// q-context 1: base_q_idx 21..=60.

/// TXB_SKIP_LUMA_64, q-context 1: the same table as [`TXB_SKIP_LUMA_64`], for `base_q_idx` 21..=60.
pub const TXB_SKIP_LUMA_64_Q1: [u16; 3] = [26584, 32768, 0];
/// TXB_SKIP_LUMA_32, q-context 1: the same table as [`TXB_SKIP_LUMA_32`], for `base_q_idx` 21..=60.
pub const TXB_SKIP_LUMA_32_Q1: [u16; 3] = [26726, 32768, 0];
/// TXB_SKIP_LUMA_16, q-context 1: the same table as [`TXB_SKIP_LUMA_16`], for `base_q_idx` 21..=60.
pub const TXB_SKIP_LUMA_16_Q1: [u16; 3] = [31901, 32768, 0];
/// TXB_SKIP_CHROMA_8, q-context 1: the same table as [`TXB_SKIP_CHROMA_8`], for `base_q_idx` 21..=60.
pub const TXB_SKIP_CHROMA_8_Q1: [[u16; 3]; 3] =
    [[4419, 32768, 0], [16294, 32768, 0], [28345, 32768, 0]];
/// TXB_SKIP_CHROMA_16, q-context 1: the same table as [`TXB_SKIP_CHROMA_16`], for `base_q_idx` 21..=60.
pub const TXB_SKIP_CHROMA_16_Q1: [[u16; 3]; 3] =
    [[4215, 32768, 0], [15756, 32768, 0], [28341, 32768, 0]];
/// TXB_SKIP_CHROMA_32, q-context 1: the same table as [`TXB_SKIP_CHROMA_32`], for `base_q_idx` 21..=60.
pub const TXB_SKIP_CHROMA_32_Q1: [[u16; 3]; 3] =
    [[5583, 32768, 0], [21313, 32768, 0], [29390, 32768, 0]];
/// EOB_EXTRA_LUMA_16, q-context 1: the same table as [`EOB_EXTRA_LUMA_16`], for `base_q_idx` 21..=60.
pub const EOB_EXTRA_LUMA_16_Q1: [[u16; 3]; 9] = [
    [22409, 32768, 0],
    [21012, 32768, 0],
    [15650, 32768, 0],
    [17395, 32768, 0],
...
/// EOB_EXTRA_LUMA_32, q-context 1: the same table as [`EOB_EXTRA_LUMA_32`], for `base_q_idx` 21..=60.
pub const EOB_EXTRA_LUMA_32_Q1: [[u16; 3]; 9] = [
    [25991, 32768, 0],
    [20314, 32768, 0],
    [17731, 32768, 0],
    [19678, 32768, 0],
...
/// EOB_EXTRA_LUMA_64, q-context 1: the same table as [`EOB_EXTRA_LUMA_64`], for `base_q_idx` 21..=60.
pub const EOB_EXTRA_LUMA_64_Q1: [[u16; 3]; 9] = [
    [26605, 32768, 0],
    [11304, 32768, 0],
    [16726, 32768, 0],
    [16560, 32768, 0],
...
/// EOB_EXTRA_CHROMA_8, q-context 1: the same table as [`EOB_EXTRA_CHROMA_8`], for `base_q_idx` 21..=60.
pub const EOB_EXTRA_CHROMA_8_Q1: [[u16; 3]; 9] = [
    [22117, 32768, 0],
    [22028, 32768, 0],
    [18650, 32768, 0],
    [16042, 32768, 0],
...
/// EOB_EXTRA_CHROMA_16, q-context 1: the same table as [`EOB_EXTRA_CHROMA_16`], for `base_q_idx` 21..=60.
pub const EOB_EXTRA_CHROMA_16_Q1: [[u16; 3]; 9] = [
    [24220, 32768, 0],
    [22480, 32768, 0],
    [17737, 32768, 0],
    [18916, 32768, 0],
...
/// EOB_EXTRA_CHROMA_32, q-context 1: the same table as [`EOB_EXTRA_CHROMA_32`], for `base_q_idx` 21..=60.
pub const EOB_EXTRA_CHROMA_32_Q1: [[u16; 3]; 9] = [
    [26585, 32768, 0],
    [21469, 32768, 0],
    [20432, 32768, 0],
    [17735, 32768, 0],
...
/// EOB_PT_1024_LUMA, q-context 1: the same table as [`EOB_PT_1024_LUMA`], for `base_q_idx` 21..=60.
pub const EOB_PT_1024_LUMA_Q1: [u16; 12] = [
    696, 948, 3145, 5702, 9706, 13217, 17851, 21856, 25692, 28034, 32768, 0,
];
/// EOB_PT_1024_CHROMA, q-context 1: the same table as [`EOB_PT_1024_CHROMA`], for `base_q_idx` 21..=60.
pub const EOB_PT_1024_CHROMA_Q1: [u16; 12] = [
    2672, 3591, 9330, 17084, 22725, 24284, 26527, 28027, 28377, 30876, 32768, 0,
];
/// EOB_PT_256_LUMA, q-context 1: the same table as [`EOB_PT_256_LUMA`], for `base_q_idx` 21..=60.
pub const EOB_PT_256_LUMA_Q1: [u16; 10] =
    [1448, 2109, 4151, 6263, 9329, 13260, 17944, 23300, 32768, 0];
/// EOB_PT_256_CHROMA, q-context 1: the same table as [`EOB_PT_256_CHROMA`], for `base_q_idx` 21..=60.
pub const EOB_PT_256_CHROMA_Q1: [u16; 10] = [
    6402, 8148, 12623, 15072, 18728, 22847, 26447, 29377, 32768, 0,
];
/// EOB_PT_64_CHROMA, q-context 1: the same table as [`EOB_PT_64_CHROMA`], for `base_q_idx` 21..=60.
pub const EOB_PT_64_CHROMA_Q1: [u16; 8] = [8609, 10612, 14624, 18714, 22614, 29024, 32768, 0];
/// COEFF_BASE_LUMA_16, q-context 1: the same table as [`COEFF_BASE_LUMA_16`], for `base_q_idx` 21..=60.
pub const COEFF_BASE_LUMA_16_Q1: [[u16; 5]; 42] = [
...
/// COEFF_BASE_LUMA_32, q-context 1: the same table as [`COEFF_BASE_LUMA_32`], for `base_q_idx` 21..=60.
pub const COEFF_BASE_LUMA_32_Q1: [[u16; 5]; 42] = [
...
/// COEFF_BASE_LUMA_64, q-context 1: the same table as [`COEFF_BASE_LUMA_64`], for `base_q_idx` 21..=60.
pub const COEFF_BASE_LUMA_64_Q1: [[u16; 5]; 42] = [
...
/// COEFF_BASE_CHROMA_8, q-context 1: the same table as [`COEFF_BASE_CHROMA_8`], for `base_q_idx` 21..=60.
pub const COEFF_BASE_CHROMA_8_Q1: [[u16; 5]; 42] = [
...
/// COEFF_BASE_CHROMA_16, q-context 1: the same table as [`COEFF_BASE_CHROMA_16`], for `base_q_idx` 21..=60.
pub const COEFF_BASE_CHROMA_16_Q1: [[u16; 5]; 42] = [
...
/// COEFF_BASE_CHROMA_32, q-context 1: the same table as [`COEFF_BASE_CHROMA_32`], for `base_q_idx` 21..=60.
pub const COEFF_BASE_CHROMA_32_Q1: [[u16; 5]; 42] = [
...
/// COEFF_BR_LUMA_16, q-context 1: the same table as [`COEFF_BR_LUMA_16`], for `base_q_idx` 21..=60.
pub const COEFF_BR_LUMA_16_Q1: [[u16; 5]; 21] = [
...
/// COEFF_BR_LUMA_32, q-context 1: the same table as [`COEFF_BR_LUMA_32`], for `base_q_idx` 21..=60.
pub const COEFF_BR_LUMA_32_Q1: [[u16; 5]; 21] = [
...
/// COEFF_BR_CHROMA_8, q-context 1: the same table as [`COEFF_BR_CHROMA_8`], for `base_q_idx` 21..=60.
pub const COEFF_BR_CHROMA_8_Q1: [[u16; 5]; 21] = [
...
/// COEFF_BR_CHROMA_16, q-context 1: the same table as [`COEFF_BR_CHROMA_16`], for `base_q_idx` 21..=60.
pub const COEFF_BR_CHROMA_16_Q1: [[u16; 5]; 21] = [
...
/// COEFF_BR_CHROMA_32, q-context 1: the same table as [`COEFF_BR_CHROMA_32`], for `base_q_idx` 21..=60.
pub const COEFF_BR_CHROMA_32_Q1: [[u16; 5]; 21] = [
...
/// COEFF_BASE_EOB_LUMA_16, q-context 1: the same table as [`COEFF_BASE_EOB_LUMA_16`], for `base_q_idx` 21..=60.
pub const COEFF_BASE_EOB_LUMA_16_Q1: [[u16; 4]; 4] = [
    [2644, 25198, 32768, 0],
    [32038, 32451, 32768, 0],
    [32639, 32695, 32768, 0],
    [32166, 32518, 32768, 0],
];
/// COEFF_BASE_EOB_LUMA_32, q-context 1: the same table as [`COEFF_BASE_EOB_LUMA_32`], for `base_q_idx` 21..=60.
pub const COEFF_BASE_EOB_LUMA_32_Q1: [[u16; 4]; 4] = [
    [1044, 2257, 32768, 0],
    [30755, 31923, 32768, 0],
    [32208, 32693, 32768, 0],
    [32244, 32615, 32768, 0],
];
/// COEFF_BASE_EOB_LUMA_64, q-context 1: the same table as [`COEFF_BASE_EOB_LUMA_64`], for `base_q_idx` 21..=60.
pub const COEFF_BASE_EOB_LUMA_64_Q1: [[u16; 4]; 4] = [
    [478, 1834, 32768, 0],
    [31005, 31987, 32768, 0],
    [32317, 32724, 32768, 0],
    [30865, 32648, 32768, 0],
];
/// COEFF_BASE_EOB_CHROMA_8, q-context 1: the same table as [`COEFF_BASE_EOB_CHROMA_8`], for `base_q_idx` 21..=60.
pub const COEFF_BASE_EOB_CHROMA_8_Q1: [[u16; 4]; 4] = [
    [26279, 30968, 32768, 0],
    [31142, 32495, 32768, 0],
    [31713, 32540, 32768, 0],
    [31929, 32594, 32768, 0],
];
/// COEFF_BASE_EOB_CHROMA_16, q-context 1: the same table as [`COEFF_BASE_EOB_CHROMA_16`], for `base_q_idx` 21..=60.
pub const COEFF_BASE_EOB_CHROMA_16_Q1: [[u16; 4]; 4] = [
    [17187, 27668, 32768, 0],
    [31714, 32550, 32768, 0],
    [32283, 32678, 32768, 0],
    [31930, 32563, 32768, 0],
];
/// COEFF_BASE_EOB_CHROMA_32, q-context 1: the same table as [`COEFF_BASE_EOB_CHROMA_32`], for `base_q_idx` 21..=60.
pub const COEFF_BASE_EOB_CHROMA_32_Q1: [[u16; 4]; 4] = [
    [21317, 26207, 32768, 0],
    [29133, 30868, 32768, 0],
    [29311, 31231, 32768, 0],
    [29657, 31087, 32768, 0],
...
// q-context 3: base_q_idx 121..=255.

/// TXB_SKIP_LUMA_64, q-context 3: the same table as [`TXB_SKIP_LUMA_64`], for `base_q_idx` 121..=255.
pub const TXB_SKIP_LUMA_64_Q3: [u16; 3] = [31539, 32768, 0];
/// TXB_SKIP_LUMA_32, q-context 3: the same table as [`TXB_SKIP_LUMA_32`], for `base_q_idx` 121..=255.
pub const TXB_SKIP_LUMA_32_Q3: [u16; 3] = [31671, 32768, 0];
/// TXB_SKIP_LUMA_16, q-context 3: the same table as [`TXB_SKIP_LUMA_16`], for `base_q_idx` 121..=255.
pub const TXB_SKIP_LUMA_16_Q3: [u16; 3] = [32510, 32768, 0];
/// TXB_SKIP_CHROMA_8, q-context 3: the same table as [`TXB_SKIP_CHROMA_8`], for `base_q_idx` 121..=255.
pub const TXB_SKIP_CHROMA_8_Q3: [[u16; 3]; 3] =
    [[2786, 32768, 0], [11194, 32768, 0], [20155, 32768, 0]];
...
/// EOB_EXTRA_LUMA_32, q-context 3: the same table as [`EOB_EXTRA_LUMA_32`], for `base_q_idx` 121..=255.
pub const EOB_EXTRA_LUMA_32_Q3: [[u16; 3]; 9] = [
    [21442, 32768, 0],
    [22358, 32768, 0],
    [18503, 32768, 0],
    [20291, 32768, 0],
...
/// EOB_EXTRA_LUMA_64, q-context 3: the same table as [`EOB_EXTRA_LUMA_64`], for `base_q_idx` 121..=255.
pub const EOB_EXTRA_LUMA_64_Q3: [[u16; 3]; 9] = [
    [26064, 32768, 0],
    [22098, 32768, 0],
    [19613, 32768, 0],
    [20525, 32768, 0],
...
/// EOB_EXTRA_CHROMA_8, q-context 3: the same table as [`EOB_EXTRA_CHROMA_8`], for `base_q_idx` 121..=255.
pub const EOB_EXTRA_CHROMA_8_Q3: [[u16; 3]; 9] = [
    [20125, 32768, 0],
    [20559, 32768, 0],
    [21707, 32768, 0],
    [22296, 32768, 0],
...
/// EOB_EXTRA_CHROMA_16, q-context 3: the same table as [`EOB_EXTRA_CHROMA_16`], for `base_q_idx` 121..=255.
pub const EOB_EXTRA_CHROMA_16_Q3: [[u16; 3]; 9] = [
    [22752, 32768, 0],
    [25006, 32768, 0],
    [22075, 32768, 0],
    [21576, 32768, 0],
...
/// EOB_EXTRA_CHROMA_32, q-context 3: the same table as [`EOB_EXTRA_CHROMA_32`], for `base_q_idx` 121..=255.
pub const EOB_EXTRA_CHROMA_32_Q3: [[u16; 3]; 9] = [
    [24648, 32768, 0],
    [24949, 32768, 0],
    [20708, 32768, 0],
    [23905, 32768, 0],
...
/// EOB_PT_1024_LUMA, q-context 3: the same table as [`EOB_PT_1024_LUMA`], for `base_q_idx` 121..=255.
pub const EOB_PT_1024_LUMA_Q3: [u16; 12] = [
    6698, 8334, 11961, 15762, 20186, 23862, 27434, 29326, 31082, 32050, 32768, 0,
];
/// EOB_PT_1024_CHROMA, q-context 3: the same table as [`EOB_PT_1024_CHROMA`], for `base_q_idx` 121..=255.
pub const EOB_PT_1024_CHROMA_Q3: [u16; 12] = [
    20569, 22426, 25569, 26859, 28053, 28913, 29486, 29724, 29807, 32570, 32768, 0,
];
/// EOB_PT_256_LUMA, q-context 3: the same table as [`EOB_PT_256_LUMA`], for `base_q_idx` 121..=255.
pub const EOB_PT_256_LUMA_Q3: [u16; 10] = [
    5348, 7113, 11820, 15924, 22106, 26777, 30334, 31757, 32768, 0,
];
/// EOB_PT_256_CHROMA, q-context 3: the same table as [`EOB_PT_256_CHROMA`], for `base_q_idx` 121..=255.
pub const EOB_PT_256_CHROMA_Q3: [u16; 10] = [
    23110, 24597, 27140, 28894, 30167, 30927, 31392, 32094, 32768, 0,
];
/// EOB_PT_64_CHROMA, q-context 3: the same table as [`EOB_PT_64_CHROMA`], for `base_q_idx` 121..=255.
pub const EOB_PT_64_CHROMA_Q3: [u16; 8] = [24212, 25708, 28268, 30035, 31307, 32049, 32768, 0];
/// COEFF_BASE_LUMA_16, q-context 3: the same table as [`COEFF_BASE_LUMA_16`], for `base_q_idx` 121..=255.
pub const COEFF_BASE_LUMA_16_Q3: [[u16; 5]; 42] = [
...
/// COEFF_BASE_LUMA_32, q-context 3: the same table as [`COEFF_BASE_LUMA_32`], for `base_q_idx` 121..=255.
pub const COEFF_BASE_LUMA_32_Q3: [[u16; 5]; 42] = [
...
/// COEFF_BASE_LUMA_64, q-context 3: the same table as [`COEFF_BASE_LUMA_64`], for `base_q_idx` 121..=255.
pub const COEFF_BASE_LUMA_64_Q3: [[u16; 5]; 42] = [
...
/// COEFF_BASE_CHROMA_8, q-context 3: the same table as [`COEFF_BASE_CHROMA_8`], for `base_q_idx` 121..=255.
pub const COEFF_BASE_CHROMA_8_Q3: [[u16; 5]; 42] = [
...
/// COEFF_BASE_CHROMA_16, q-context 3: the same table as [`COEFF_BASE_CHROMA_16`], for `base_q_idx` 121..=255.
pub const COEFF_BASE_CHROMA_16_Q3: [[u16; 5]; 42] = [
...
/// COEFF_BASE_CHROMA_32, q-context 3: the same table as [`COEFF_BASE_CHROMA_32`], for `base_q_idx` 121..=255.
pub const COEFF_BASE_CHROMA_32_Q3: [[u16; 5]; 42] = [
...
/// COEFF_BR_LUMA_16, q-context 3: the same table as [`COEFF_BR_LUMA_16`], for `base_q_idx` 121..=255.
pub const COEFF_BR_LUMA_16_Q3: [[u16; 5]; 21] = [
...
/// COEFF_BR_LUMA_32, q-context 3: the same table as [`COEFF_BR_LUMA_32`], for `base_q_idx` 121..=255.
pub const COEFF_BR_LUMA_32_Q3: [[u16; 5]; 21] = [
...
/// COEFF_BR_CHROMA_8, q-context 3: the same table as [`COEFF_BR_CHROMA_8`], for `base_q_idx` 121..=255.
pub const COEFF_BR_CHROMA_8_Q3: [[u16; 5]; 21] = [
...
/// COEFF_BR_CHROMA_16, q-context 3: the same table as [`COEFF_BR_CHROMA_16`], for `base_q_idx` 121..=255.
pub const COEFF_BR_CHROMA_16_Q3: [[u16; 5]; 21] = [
...
/// COEFF_BR_CHROMA_32, q-context 3: the same table as [`COEFF_BR_CHROMA_32`], for `base_q_idx` 121..=255.
pub const COEFF_BR_CHROMA_32_Q3: [[u16; 5]; 21] = [
...
/// COEFF_BASE_EOB_LUMA_16, q-context 3: the same table as [`COEFF_BASE_EOB_LUMA_16`], for `base_q_idx` 121..=255.
pub const COEFF_BASE_EOB_LUMA_16_Q3: [[u16; 4]; 4] = [
    [19980, 30591, 32768, 0],
    [32219, 32597, 32768, 0],
    [32581, 32706, 32768, 0],
    [31803, 32287, 32768, 0],
];
/// COEFF_BASE_EOB_LUMA_32, q-context 3: the same table as [`COEFF_BASE_EOB_LUMA_32`], for `base_q_idx` 121..=255.
pub const COEFF_BASE_EOB_LUMA_32_Q3: [[u16; 4]; 4] = [
    [24647, 30463, 32768, 0],
    [32412, 32695, 32768, 0],
    [32468, 32720, 32768, 0],
    [31269, 32523, 32768, 0],
];
/// COEFF_BASE_EOB_LUMA_64, q-context 3: the same table as [`COEFF_BASE_EOB_LUMA_64`], for `base_q_idx` 121..=255.
pub const COEFF_BASE_EOB_LUMA_64_Q3: [[u16; 4]; 4] = [
    [12358, 24977, 32768, 0],
    [31331, 32385, 32768, 0],
    [32634, 32756, 32768, 0],
    [30411, 32548, 32768, 0],
];
/// COEFF_BASE_EOB_CHROMA_8, q-context 3: the same table as [`COEFF_BASE_EOB_CHROMA_8`], for `base_q_idx` 121..=255.
pub const COEFF_BASE_EOB_CHROMA_8_Q3: [[u16; 4]; 4] = [
    [27558, 31151, 32768, 0],
    [32020, 32640, 32768, 0],
    [32097, 32575, 32768, 0],
    [32242, 32719, 32768, 0],
];
/// COEFF_BASE_EOB_CHROMA_16, q-context 3: the same table as [`COEFF_BASE_EOB_CHROMA_16`], for `base_q_idx` 121..=255.
pub const COEFF_BASE_EOB_CHROMA_16_Q3: [[u16; 4]; 4] = [
    [26473, 30507, 32768, 0],
    [32431, 32723, 32768, 0],
    [32196, 32611, 32768, 0],
    [31588, 32528, 32768, 0],
];
/// COEFF_BASE_EOB_CHROMA_32, q-context 3: the same table as [`COEFF_BASE_EOB_CHROMA_32`], for `base_q_idx` 121..=255.
pub const COEFF_BASE_EOB_CHROMA_32_Q3: [[u16; 4]; 4] = [
    [28482, 31505, 32768, 0],
    [32152, 32701, 32768, 0],
    [31732, 32598, 32768, 0],
    [31767, 32712, 32768, 0],
...
// TX_8X8/TX_4X4 tables (lane-av1-rect): the 8x8 luma and 4x4 chroma coefficient
// coding CDFs, extracted byte-exact from libaom's Default_*_Cdf tables, needed
// to let a key frame's straddling 16x16 quadrant split down to an 8x8 leaf.

/// TXB_SKIP_LUMA_8_Q0, q-context 0: the same table as [`TXB_SKIP_LUMA_8`], for `base_q_idx` 0..=20.
pub const TXB_SKIP_LUMA_8_Q0: [u16; 3] = [31548, 32768, 0];

/// TXB_SKIP_LUMA_8_Q1, q-context 1: the same table as [`TXB_SKIP_LUMA_8`], for `base_q_idx` 21..=60.
pub const TXB_SKIP_LUMA_8_Q1: [u16; 3] = [31782, 32768, 0];

/// the all-zero flag of an 8x8 luma transform, whose only context is zero because the transform covers its whole block.
pub const TXB_SKIP_LUMA_8: [u16; 3] = [31957, 32768, 0];
...
/// TXB_SKIP_CHROMA_4_Q1, q-context 1: the same table as [`TXB_SKIP_CHROMA_4`], for `base_q_idx` 21..=60.
pub const TXB_SKIP_CHROMA_4_Q1: [[u16; 3]; 3] =
    [[5495, 32768, 0], [17942, 32768, 0], [28280, 32768, 0]];

/// the all-zero flag of a 4x4 chroma transform.
pub const TXB_SKIP_CHROMA_4: [[u16; 3]; 3] =
    [[4119, 32768, 0], [16026, 32768, 0], [25657, 32768, 0]];

/// TXB_SKIP_CHROMA_4_Q3, q-context 3: the same table as [`TXB_SKIP_CHROMA_4`], for `base_q_idx` 121..=255.
pub const TXB_SKIP_CHROMA_4_Q3: [[u16; 3]; 3] =
    [[2713, 32768, 0], [11861, 32768, 0], [20773, 32768, 0]];

/// EOB_PT_64_LUMA_Q0, q-context 0: the same table as [`EOB_PT_64_LUMA`], for `base_q_idx` 0..=20.
pub const EOB_PT_64_LUMA_Q0: [u16; 8] = [329, 498, 1101, 1784, 3265, 7758, 32768, 0];

/// EOB_PT_64_LUMA_Q1, q-context 1: the same table as [`EOB_PT_64_LUMA`], for `base_q_idx` 21..=60.
pub const EOB_PT_64_LUMA_Q1: [u16; 8] = [1260, 1446, 2253, 3712, 6652, 13369, 32768, 0];

/// the end-of-block group of an 8x8 luma transform.
pub const EOB_PT_64_LUMA: [u16; 8] = [2374, 2772, 4583, 7276, 12288, 19706, 32768, 0];
...
/// EOB_PT_16_CHROMA_Q1, q-context 1: the same table as [`EOB_PT_16_CHROMA`], for `base_q_idx` 21..=60.
pub const EOB_PT_16_CHROMA_Q1: [u16; 6] = [7637, 9498, 14259, 19108, 32768, 0];

/// the end-of-block group of a 4x4 chroma transform.
pub const EOB_PT_16_CHROMA: [u16; 6] = [11139, 13270, 18241, 23566, 32768, 0];

/// EOB_PT_16_CHROMA_Q3, q-context 3: the same table as [`EOB_PT_16_CHROMA`], for `base_q_idx` 121..=255.
pub const EOB_PT_16_CHROMA_Q3: [u16; 6] = [19575, 21766, 26044, 29709, 32768, 0];

/// EOB_EXTRA_LUMA_8_Q0, q-context 0: the same table as [`EOB_EXTRA_LUMA_8`], for `base_q_idx` 0..=20.
pub const EOB_EXTRA_LUMA_8_Q0: [[u16; 3]; 9] = [
    [20401, 32768, 0],
    [17025, 32768, 0],
    [12845, 32768, 0],
    [12873, 32768, 0],
...
/// EOB_EXTRA_LUMA_8_Q1, q-context 1: the same table as [`EOB_EXTRA_LUMA_8`], for `base_q_idx` 21..=60.
pub const EOB_EXTRA_LUMA_8_Q1: [[u16; 3]; 9] = [
    [20430, 32768, 0],
    [20662, 32768, 0],
    [15367, 32768, 0],
    [16970, 32768, 0],
...
/// the top offset bit inside an 8x8 luma transform's end-of-block group.
pub const EOB_EXTRA_LUMA_8: [[u16; 3]; 9] = [
    [19139, 32768, 0],
    [21487, 32768, 0],
    [18959, 32768, 0],
    [20910, 32768, 0],
...
/// EOB_EXTRA_LUMA_8_Q3, q-context 3: the same table as [`EOB_EXTRA_LUMA_8`], for `base_q_idx` 121..=255.
pub const EOB_EXTRA_LUMA_8_Q3: [[u16; 3]; 9] = [
    [20238, 32768, 0],
    [21057, 32768, 0],
    [19159, 32768, 0],
    [22337, 32768, 0],
...
/// EOB_EXTRA_CHROMA_4_Q0, q-context 0: the same table as [`EOB_EXTRA_CHROMA_4`], for `base_q_idx` 0..=20.
pub const EOB_EXTRA_CHROMA_4_Q0: [[u16; 3]; 9] = [
    [19069, 32768, 0],
    [22525, 32768, 0],
    [13377, 32768, 0],
    [16384, 32768, 0],
...
/// EOB_EXTRA_CHROMA_4_Q1, q-context 1: the same table as [`EOB_EXTRA_CHROMA_4`], for `base_q_idx` 21..=60.
pub const EOB_EXTRA_CHROMA_4_Q1: [[u16; 3]; 9] = [
    [20335, 32768, 0],
    [21667, 32768, 0],
    [14818, 32768, 0],
    [16384, 32768, 0],
...
pub const EOB_EXTRA_CHROMA_4: [[u16; 3]; 9] = [
    [20090, 32768, 0],
    [19444, 32768, 0],
    [17286, 32768, 0],
    [16384, 32768, 0],
...
/// EOB_EXTRA_CHROMA_4_Q3, q-context 3: the same table as [`EOB_EXTRA_CHROMA_4`], for `base_q_idx` 121..=255.
pub const EOB_EXTRA_CHROMA_4_Q3: [[u16; 3]; 9] = [
    [21416, 32768, 0],
    [20855, 32768, 0],
    [23410, 32768, 0],
    [16384, 32768, 0],
...
/// COEFF_BASE_LUMA_8_Q0, q-context 0: the same table as [`COEFF_BASE_LUMA_8`], for `base_q_idx` 0..=20.
pub const COEFF_BASE_LUMA_8_Q0: [[u16; 5]; 42] = [
...
/// COEFF_BASE_LUMA_8_Q1, q-context 1: the same table as [`COEFF_BASE_LUMA_8`], for `base_q_idx` 21..=60.
pub const COEFF_BASE_LUMA_8_Q1: [[u16; 5]; 42] = [
...
/// an 8x8 luma coefficient's base level.
pub const COEFF_BASE_LUMA_8: [[u16; 5]; 42] = [
...
/// COEFF_BASE_LUMA_8_Q3, q-context 3: the same table as [`COEFF_BASE_LUMA_8`], for `base_q_idx` 121..=255.
pub const COEFF_BASE_LUMA_8_Q3: [[u16; 5]; 42] = [
...
/// COEFF_BASE_CHROMA_4_Q0, q-context 0: the same table as [`COEFF_BASE_CHROMA_4`], for `base_q_idx` 0..=20.
pub const COEFF_BASE_CHROMA_4_Q0: [[u16; 5]; 42] = [
...
/// COEFF_BASE_CHROMA_4_Q1, q-context 1: the same table as [`COEFF_BASE_CHROMA_4`], for `base_q_idx` 21..=60.
pub const COEFF_BASE_CHROMA_4_Q1: [[u16; 5]; 42] = [
...
/// a 4x4 chroma coefficient's base level.
pub const COEFF_BASE_CHROMA_4: [[u16; 5]; 42] = [
...
/// COEFF_BASE_CHROMA_4_Q3, q-context 3: the same table as [`COEFF_BASE_CHROMA_4`], for `base_q_idx` 121..=255.
pub const COEFF_BASE_CHROMA_4_Q3: [[u16; 5]; 42] = [
...
/// COEFF_BASE_EOB_LUMA_8_Q0, q-context 0: the same table as [`COEFF_BASE_EOB_LUMA_8`], for `base_q_idx` 0..=20.
pub const COEFF_BASE_EOB_LUMA_8_Q0: [[u16; 4]; 4] = [
    [5717, 26477, 32768, 0],
    [30491, 31703, 32768, 0],
    [31550, 32158, 32768, 0],
    [29648, 31491, 32768, 0],
...
/// COEFF_BASE_EOB_LUMA_8_Q1, q-context 1: the same table as [`COEFF_BASE_EOB_LUMA_8`], for `base_q_idx` 21..=60.
pub const COEFF_BASE_EOB_LUMA_8_Q1: [[u16; 4]; 4] = [
    [15239, 29932, 32768, 0],
    [31315, 32095, 32768, 0],
    [32130, 32434, 32768, 0],
    [30864, 31996, 32768, 0],
...
/// the last coefficient's base level, for an 8x8 luma transform.
pub const COEFF_BASE_EOB_LUMA_8: [[u16; 4]; 4] = [
    [18049, 30489, 32768, 0],
    [31706, 32286, 32768, 0],
    [32163, 32473, 32768, 0],
    [31550, 32184, 32768, 0],
...
/// COEFF_BASE_EOB_LUMA_8_Q3, q-context 3: the same table as [`COEFF_BASE_EOB_LUMA_8`], for `base_q_idx` 121..=255.
pub const COEFF_BASE_EOB_LUMA_8_Q3: [[u16; 4]; 4] = [
    [21457, 31043, 32768, 0],
    [31951, 32483, 32768, 0],
    [32153, 32562, 32768, 0],
    [31473, 32215, 32768, 0],
...
/// COEFF_BASE_EOB_CHROMA_4_Q0, q-context 0: the same table as [`COEFF_BASE_EOB_CHROMA_4`], for `base_q_idx` 0..=20.
pub const COEFF_BASE_EOB_CHROMA_4_Q0: [[u16; 4]; 4] = [
    [21365, 30026, 32768, 0],
    [30512, 32423, 32768, 0],
    [31658, 32621, 32768, 0],
    [29630, 31881, 32768, 0],
...
/// COEFF_BASE_EOB_CHROMA_4_Q1, q-context 1: the same table as [`COEFF_BASE_EOB_CHROMA_4`], for `base_q_idx` 21..=60.
pub const COEFF_BASE_EOB_CHROMA_4_Q1: [[u16; 4]; 4] = [
    [26594, 31212, 32768, 0],
    [31208, 32582, 32768, 0],
    [31835, 32637, 32768, 0],
    [30595, 32206, 32768, 0],
...
/// the same, for a 4x4 chroma transform.
pub const COEFF_BASE_EOB_CHROMA_4: [[u16; 4]; 4] = [
    [27258, 31095, 32768, 0],
    [31804, 32623, 32768, 0],
    [31763, 32528, 32768, 0],
    [31438, 32506, 32768, 0],
...
/// COEFF_BASE_EOB_CHROMA_4_Q3, q-context 3: the same table as [`COEFF_BASE_EOB_CHROMA_4`], for `base_q_idx` 121..=255.
pub const COEFF_BASE_EOB_CHROMA_4_Q3: [[u16; 4]; 4] = [
    [27877, 31584, 32768, 0],
    [32170, 32728, 32768, 0],
    [32155, 32688, 32768, 0],
    [32219, 32702, 32768, 0],
...
/// COEFF_BR_LUMA_8_Q0, q-context 0: the same table as [`COEFF_BR_LUMA_8`], for `base_q_idx` 0..=20.
pub const COEFF_BR_LUMA_8_Q0: [[u16; 5]; 21] = [
...
/// COEFF_BR_LUMA_8_Q1, q-context 1: the same table as [`COEFF_BR_LUMA_8`], for `base_q_idx` 21..=60.
pub const COEFF_BR_LUMA_8_Q1: [[u16; 5]; 21] = [
...
pub const COEFF_BR_LUMA_8: [[u16; 5]; 21] = [
...
/// COEFF_BR_LUMA_8_Q3, q-context 3: the same table as [`COEFF_BR_LUMA_8`], for `base_q_idx` 121..=255.
pub const COEFF_BR_LUMA_8_Q3: [[u16; 5]; 21] = [
...
/// COEFF_BR_CHROMA_4_Q0, q-context 0: the same table as [`COEFF_BR_CHROMA_4`], for `base_q_idx` 0..=20.
pub const COEFF_BR_CHROMA_4_Q0: [[u16; 5]; 21] = [
...
/// COEFF_BR_CHROMA_4_Q1, q-context 1: the same table as [`COEFF_BR_CHROMA_4`], for `base_q_idx` 21..=60.
pub const COEFF_BR_CHROMA_4_Q1: [[u16; 5]; 21] = [
...
pub const COEFF_BR_CHROMA_4: [[u16; 5]; 21] = [
...
/// COEFF_BR_CHROMA_4_Q3, q-context 3: the same table as [`COEFF_BR_CHROMA_4`], for `base_q_idx` 121..=255.
pub const COEFF_BR_CHROMA_4_Q3: [[u16; 5]; 21] = [
...
// The tables below extend the `_LUMA_8`/`Luma8Set1` straddling-16x16 support
// above to a resolved `TX4` (spec `TX_4X4`) luma transform unit: a
// `TxMode::Select` 8x8 leaf or 16x16 block whose `tx_depth` read exhausts
// down to the smallest size (lane-av1tx4). Extracted byte-exact from
// libaom's `token_cdfs.h` `TX_SIZES` idx0 (`TX_4X4`)/`PLANE_TYPES` idx0
// (luma), the same indexing method that verified the `_LUMA_8`/`COEFF_BR_LUMA_8`
// families above.

/// The full luma sub-range of spec `TXB_SKIP_CONTEXTS` (contexts 0..6) for a
/// resolved 4x4 luma transform, same convention as [`TXB_SKIP_LUMA_8_CTX`].
pub const TXB_SKIP_LUMA_4_Q0_CTX: [[u16; 3]; 7] = [
    [31849, 32768, 0],
    [5892, 32768, 0],
    [12112, 32768, 0],
    [21935, 32768, 0],
...
/// `Default_Intra_Ext_Tx_Cdf`'s `TX_SET_INTRA_1` row for `TX_4X4` (spec
/// 9.4): the transform type of an intra 4x4 transform when `reduced_tx_set`
/// is false. Same `Luma8`/`Luma8Set1` split as [`INTRA_TX_TYPE_SET1_8`],
/// kept named for the day a non-reduced path exists.
pub const INTRA_TX_TYPE_SET1_4: [[u16; 8]; 13] = [
...
// lane-gmaffine r4: the `TX_CLASS_HORIZ`/`TX_CLASS_VERT` (`eob_multi_ctx == 1`)
// half of libaom's `av1_default_eob_multi*_cdfs[q][plane][2]` for every eob_pt
// table that carried only its 2D row -- an inter block's chroma inherits the
// colocated luma `tx_type` verbatim, so a 1D-class chroma TU is routine and
// used to read the 2D CDF (right value, wrong interval).

/// `EOB_PT_16_CHROMA`'s class-1 sibling (`av1_default_eob_multi16_cdfs[q][1][1]`).
...
/// `EOB_PT_64_CHROMA`'s class-1 sibling (`av1_default_eob_multi64_cdfs[q][1][1]`).
...
pub const EOB_PT_64_CHROMA_CLASS1_Q3: [u16; 8] = [8726, 12378, 19409, 26450, 30038, 32462, 32768, 0];
...
/// `EOB_PT_256_LUMA`'s class-1 sibling (`av1_default_eob_multi256_cdfs[q][0][1]`).
pub const EOB_PT_256_LUMA_CLASS1_Q0: [u16; 10] = [998, 1850, 2998, 5604, 17341, 19888, 22899, 25583, 32768, 0];
pub const EOB_PT_256_LUMA_CLASS1_Q1: [u16; 10] = [399, 1019, 1749, 3038, 10444, 15546, 22739, 27294, 32768, 0];
pub const EOB_PT_256_LUMA_CLASS1: [u16; 10] = [1084, 2358, 3488, 5122, 11483, 18103, 26023, 29799, 32768, 0];
pub const EOB_PT_256_LUMA_CLASS1_Q3: [u16; 10] = [2453, 4474, 6307, 8777, 16474, 22975, 29000, 31547, 32768, 0];

/// `EOB_PT_256_CHROMA`'s class-1 sibling (`av1_default_eob_multi256_cdfs[q][1][1]`).
pub const EOB_PT_256_CHROMA_CLASS1_Q0: [u16; 10] = [2203, 4130, 7435, 10739, 20652, 23681, 25609, 27261, 32768, 0];
pub const EOB_PT_256_CHROMA_CLASS1_Q1: [u16; 10] = [1674, 3252, 5734, 10159, 22397, 23802, 24821, 30940, 32768, 0];
pub const EOB_PT_256_CHROMA_CLASS1: [u16; 10] = [6571, 9610, 15516, 21826, 29092, 30829, 31842, 32708, 32768, 0];
pub const EOB_PT_256_CHROMA_CLASS1_Q3: [u16; 10] = [9998, 17661, 25178, 28097, 31308, 32038, 32403, 32695, 32768, 0];
