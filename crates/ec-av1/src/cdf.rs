...
/// `[10]`/`[11]` (lane-fi32x8 r1) add `BLOCK_32X8`/`BLOCK_8X32`'s own rows
/// (18101/20229, `default_filter_intra_cdfs` index 19/18 -- generated by
/// `scripts/extract-filter-intra-cdfs.py`): the four 32x8 (or 8x32) strips a
/// 32x32-level `PARTITION_HORZ_4`/`VERT_4` codes are inside
/// `av1_filter_intra_allowed_bsize`'s "both sides <= 32" bound, so libaom
/// reads `use_filter_intra` there too and the missing rows dropped the
/// symbol on every DC_PRED 1:4 strip.
pub const FILTER_INTRA: [[u16; 3]; 14] = [
    [4621, 32768, 0],
    [7866, 32768, 0],
    [12408, 32768, 0],
    [22343, 32768, 0],
    ...
    [18101, 32768, 0],
    [20229, 32768, 0],
    // lane-r14: the 16x16-level 1:4 pair (`default_filter_intra_cdfs` index
    // 17/16 -- BLOCK_16X4/BLOCK_4X16, generated by
    // `scripts/extract-filter-intra-cdfs.py`), classes 12/13. Both sides <= 32,
    // so libaom writes `use_filter_intra` on every DC_PRED strip of this shape.
    [10368, 32768, 0],
    [12770, 32768, 0],
];
