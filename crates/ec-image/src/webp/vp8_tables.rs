//! VP8 constant tables, generated from the RFC 6386 text by
//! `scratchpad/s40/gen_vp8_tables.py` — every value is the spec's own.
//!
//! Regenerating beats retyping: a single transposed probability shows up as a
//! plausible-looking decode that drifts from the reference several
//! macroblocks later.

/// Keyframe subblock mode probabilities, indexed [above][left][node].
...
/// Probability that a given token probability is updated in the header.
...
/// Token probabilities a keyframe starts from, [type][band][context][node].
...
/// DC dequantization factor per quantizer index.
...
/// AC dequantization factor per quantizer index.
...
/// Which probability band each coefficient position uses.
pub const COEFF_BANDS: [usize; 16] = [0, 1, 2, 3, 6, 4, 5, 6, 6, 6, 6, 6, 6, 6, 6, 7];

/// Coefficient scan order within a 4x4 block.
