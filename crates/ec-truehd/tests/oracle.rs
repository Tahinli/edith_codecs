...
use ec_truehd::{frame_length, TrueHdDecoder};
...
fn ffmpeg(args: &[&str]) -> Vec<u8> {
    let out = Command::new("ffmpeg").args(["-v", "error", "-nostdin"]).args(args).output().expect("run ffmpeg");
...
fn first_60_seconds_of_a_real_7_1_remux_are_bit_exact() {
    ...
    let track = demux.streams().iter().find(|s| s.params.codec == CodecId::TrueHd).expect("a TrueHD track");
    let stream_index = track.index;

    let reference = s32le(&ffmpeg(&[
        "-i", path.to_str().unwrap(), "-map", &format!("0:{stream_index}"), "-t", &SECONDS.to_string(),
        "-f", "s32le", "-",
    ]));
    let channels = 8;
    ...
    while ours.len() < wanted {
        ...
        let frame = decoder.decode_access_unit(&packet.data).unwrap_or_else(|e| panic!("AU {aus}: {e}"));
        aus += 1;
        if let Some(f) = frame {
    ...
    for ch in 0..channels {
        ...
        let e = (0..n).filter(|&i| ours[i * channels + ch] == reference[i * channels + ch]).count();
    ...
    assert!(total >= (SECONDS - 1) * 48_000 * channels, "only {total} samples");
    ...
    assert!(exact as f64 >= 0.9999 * total as f64, "{exact}/{total} exact, first mismatch {first:?}");
}
...
fn externally_encoded_mlp_round_trips_bit_exact() {
    ...
    for (codec, layout, channels) in [
        ("mlp", "mono", 1usize), ("mlp", "stereo", 2), ("mlp", "5.1", 6),
        ("truehd", "mono", 1), ("truehd", "stereo", 2), ("truehd", "5.1", 6),
    ] {
        let name = format!("{codec}-{layout}");
        let mlp = dir.join(format!("{name}.{}", if codec == "mlp" { "mlp" } else { "thd" }));
        let src = format!(
            "anoisesrc=color=pink:amplitude=0.3:seed=7:sample_rate=48000:duration=3,aformat=sample_fmts=s32:channel_layouts={layout}"
        );
        ffmpeg(&["-y", "-f", "lavfi", "-i", &src, "-c:a", codec, "-strict", "-2", mlp.to_str().unwrap()]);
        ...
        let (exact, total, first) = compare(&ours, &reference);
        let stats = decoder.check_stats();
        eprintln!("{name}: {exact}/{total} exact (ours {}, reference {}), first mismatch {first:?}, {stats:?}", ours.len(), reference.len());
        if stats.lossless_check_failures + stats.parity_failures + stats.restart_crc_failures != 0
            || exact != total
            || total < 2 * 48_000 * channels
        {
            failures.push(format!("{name}: {exact}/{total}, first mismatch {first:?}, {stats:?}"));
        }
    }
