...
1. `git log --oneline -3` here is 4f378bd (merged main 48b35ab).
2. The ONLY blocker is stream generation. Do not touch the decoder arm before a
   real 64-level HORZ/VERT partition exists in an aomdec `EC_TRACE=1` histogram
   (`grep -c 'bsize=12 value=[12]'` > 0). Iterate on the SOURCE with aomenc +
   aomdec alone (seconds per try), never through the test binary.
   Known-dead sources: mandelbrot fast zoom, 32-px horizontal noise bands,
   32-px vertical noise bands, testsrc2 — all 0 at cq 12/20/30/40/55.
   Known-dead flags: `--min-partition-size` / `--max-partition-size` (any value
   pair) — they prune rect entirely.
   Untried and most promising: a crop of the user's own 4K film (the shape is
