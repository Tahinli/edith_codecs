...
(`$HOME/.cache/inter16ab-base-r8.log`, COMPLETE, `1 passed; 7 failed`): 7 already FAILED before the fix
(`refusal_inventory`, `a_frame_edge_straddling_band`, 10-bit `angle_delta`, 10-bit
`split_transform_intra_block`, `class1_chroma_eob`, and BOTH 1:4 gates) — they come from r7's three unverified merges.
...
= class `parallel-flake-is-attempt-selection`; their baseline is now measured: BOTH already failed at `ba40d38`, so the fix did not cause them.
...
1. `a_real_aomenc_10bit_inter_sequence_with_filter_intra_on_an_intra_block_decodes_pixel_exact` is the ONLY gate that flipped ok->FAILED across this round's fix (baseline run complete: 1 passed / 7 failed). Own it first.
2. Own the 7 pre-existing merge reds (they are r7 merge debt, not this fix).
