...
| a frame with no mode-info grid | libaom derives the mi grid from the frame size; a 0-sized frame is unrepresentable (`compute_image_size`); no error site exists because no conformant header codes one | spec-conformant / defensive |
