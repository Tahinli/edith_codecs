...
| M3 inter frames | **implemented end-to-end; census bug FIXED; walled on token-phase context divergence at gop frame-1 MB(0,8)** — mode records/census/SPLITMV now agree read-for-read for ALL 60 MBs (model↔Rust B/CNT/SP zero-diff) after fixing the above-left third-candidate merge (decodemv.c:366 is ALIVE, not dead) and a model hasy2 clobber; SPLITMV corner-subblock clamp + chroma clamp MB-grid fixed (reconinter.c:370-374); remaining: frame-1 token divergence at MB(0,8) block-15/16 (context threading) — gop frame 1 at 52.6% vs ffmpeg; instrument traces (B/CNT/SP/IMB/STAGE) all in place | commits `151ef8ff`, `e01bec40`, `6152922e`, `e1eeec8d` |
