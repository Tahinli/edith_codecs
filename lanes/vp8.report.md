...
| M3 inter frames | **implemented end-to-end, walled on the r2c7 census count** — full inter pipeline lands (mode/MV parse incl. SPLITMV, census, MC in mc.rs w/ libvpx-fuzzed predictors, ref buffers, entropy revert, LF deltas); inter frames decode without desync, gop frame 1 at 51% vs ffmpeg; model+rust B streams agree read-for-read through MB(2,6) and diverge at MB(2,7)'s census: model cnt=[0,2,2,3] vs rust cnt=[0,3,2,3] with identical neighbours — the above-left +1 lands in NEAREST (rust) vs NEAR (model); re-read libvpx decodemv.c:307-361 against modes.rs find_near_mvs and scripts/vp8ref_model.py find_near | commit `151ef8ff` |
